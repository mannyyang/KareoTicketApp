...
function podio(config, io){
      ...
      Project.find({}).remove(function() {
        for (var i = 0; i < result.body.items.length; i++){
          ...
          for (var j = 0; j < result.body.items[i].fields.length; j++){
            fieldVals[result.body.items[i].fields[j].external_id] = result.body.items[i].fields[j].values[0].value;
          }
          ...
          var project = new Project({
            ...
            fields: fieldVals,
            stage: fieldVals.stage.text,
            priority: fieldVals.priority.text,
  ...
  podio.projects = function(req, res){
    Project.find({ "stage": {$ne: "Archived"}, "priority": {$ne: "Archived"} }).execFind(function(err, projects) {
      if (!err) {
  ...
  podio.archivedProjects = function(req, res){
    Project.find({ $or:[ {'stage': 'Archived'}, {'priority': 'Archived'} ]}).execFind(function(err, projects) {
      if (!err) {
        return res.json(projects);
      } else {
        return res.send(err);
